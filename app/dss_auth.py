...
class DSSAuth:
    ...
    def get_dss_mq_password(self):
        ...
        signature = self.get_signature(first_authentication_resp['realm'], first_authentication_resp['randomKey'])
        second_authentication_resp = self.second_authentication(signature, first_authentication_resp['randomKey'], public_key)
        secret_key = self.rsa_decrypt(second_authentication_resp['secretKey'], private_key)
        secret_vector = self.rsa_decrypt(second_authentication_resp['secretVector'], private_key)
        mq_credentials = self.get_mq_credentials(second_authentication_resp['token'])
        decrypted_pass = self.aes_decrypt(mq_credentials['data']['password'], secret_key, secret_vector)
