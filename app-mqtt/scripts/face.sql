...
CREATE TABLE IF NOT EXISTS center.face_data
(
...
ALTER TABLE IF EXISTS center.face_data
    OWNER to postgres;

COMMENT ON TABLE center.face_data
    IS 'Logs for face recognition events, range-partitioned by end_time.';

COMMENT ON COLUMN center.face_data.similar_faces
    IS 'JSON array containing metadata of matching faces found in the database.';

COMMENT ON COLUMN center.face_data.is_watchlist
